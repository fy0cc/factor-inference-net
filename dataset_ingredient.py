...
def cfg():
    ...
    if datatype=="3rdbipartite":
        dset = ThirdOrderDatasetBipartite(datapath, normalize_target=normalize_target,
...
def load_data(datapath, test_datapath, val_percentage, target, batch_size, datatype, refresh,
    ...
    if datatype=="3rdbipartite":
        ...
        if test_datapath is not None:
            testset = ThirdOrderDatasetBipartite(test_datapath, normalize_target=normalize_target,
                                                 singleton_factor=singleton_factor)
    elif datatype=="gaussianbipartite":
    ...
    if test_datapath is not None:
        ...
        testset, evalset, trainset = torch.utils.data.random_split(dset, lengths,
                                                                   generator=torch.Generator().manual_seed(42))
    testloader = dataloaderclass[datatype](testset, batch_size=batch_size, shuffle=False,
                                           pin_memory=True)  # out of sample
